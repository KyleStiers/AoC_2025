...
fn find_new_pos(current_pos:i32, movement:(char,i32)) -> i32 {
    let pos = current_pos;
...
fn main() -> io::Result<()> {
    ...
    for line_result in reader.lines() {
        ...
        if let Some(captures) = re.captures(&line) {
            ...
            pos = find_new_pos(pos, (direction.chars().next().unwrap(),ticks.parse().expect("Failed to parse")));
